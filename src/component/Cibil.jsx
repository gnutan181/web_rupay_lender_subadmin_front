...
//   const [searchvendor, setSearchvendor] = useState([]);
  ...
  const [cibils, setCibils] = useState([]);
...
//   };
  const fetchData = async () => {
    const { data } = await axiosInstance.get("/cibil/cibil-data");
    setCibils(data?.cibilData);
    return data;
  };
  ...
  useEffect(() => {
    if (cibils) {
      setItems(cibils);
      setPageCount(Math.ceil(cibils.length / itemsPerPage));
    }
  }, [cibils, itemsPerPage]);
